...
test("Check 202 No Results Found error page ", async ({ app }) => {
  ...
  //Assert
  await app.shoppingPage.error.expectNotResultErrorToHaveText(
...
test("Check 500 unknown Error Page  ", async ({ app }) => {
  ...
  //Assert
  await app.shoppingPage.error.expectContentToHaveText(
...
test("Check prev button in the paging", async ({ app }) => {
  ...
  await app.expectPageToHaveUrl(
    ...
    process.env.BASE_URL +
      "/en/shopping?query=iphone+sales&region=de-DE&offset=0"
  );
  await app.shoppingPage.item.expectItemsListToHaveCount(24);
