...
test("Check Cheapest first filter ", async ({
  app
}) => {
   //Actions
   await app.home.open()
...
test(`Check select specific "brand" filter`, async ({ app }) => {
  const expectedInfo = { name: "MacBook", itemPricing: "€", itemLink: "See all offers", itemBrand: "Apple"}
  //Actions
  await app.home.open();
...
test(`Check select multiple filters specifics "brand" and "Most expensive"`, async ({ app }) => {
  //Actions
  await app.home.open();
  await app.home.header.searchForm.inputSearchCriteria("laptop");
  await app.home.header.searchForm.clickEnterSearchField();
